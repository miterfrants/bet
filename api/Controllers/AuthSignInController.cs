...
namespace Homo.Bet.Api
{
    ...
    public class AuthSignInController : ControllerBase
    {
        ...
        public AuthSignInController(
            BargainingChipDBContext dbContext
            , CommonLocalizer localizer
            , IOptions<Homo.Bet.Api.AppSettings> appSettings)
        {
        ...
        [Route("auth-from-chrome-ext")]
        [HttpPost]
        public async Task<dynamic> authFromChromeExt([FromBody] DTOs.AuthWithSocialMedia dto)
        {
            UserInfo userInfo = await GoogleOAuthHelper.GetUserInfo(dto.Code);
            if (userInfo.sub == null)
            {
                throw new CustomException(ERROR_CODE.INVALID_CODE_FROM_SOCIAL_MEDIA, HttpStatusCode.BadRequest);
            }
            ...
            if (user == null)
            {
                user = UserDataservice.SignUpWithSocialMedia(_dbContext
            ...
            string[] roles = permissions.SelectMany(x => Newtonsoft.Json.JsonConvert.DeserializeObject<string[]>(x.Roles)).ToArray();
            string token = JWTHelper.GenerateToken(_jwtKey, _jwtExpirationMonth * 30 * 24 * 60, extraPayload, roles);
            return new { token = token, id = user.Id, firstName = user.FirstName, lastName = user.LastName, profile = user.Profile };
        }
    }
}